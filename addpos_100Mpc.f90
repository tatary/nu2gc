!!$ ifort -o addpos.out addpos.f90
  PROGRAM addpos
    ...
    INTEGER, PARAMETER :: TNWAVE = 34 !18    ! NWAVE*2
!   INTEGER, PARAMETER :: N_FILE = 2
    ...
    CHARACTER(LEN=10) :: cz
    CHARACTER(LEN=50) :: fbase = '/home/makiya/nugc/data/SMGs/'
!   CHARACTER(LEN=50) :: fend(N_FILE) = (/'400Mpc_LyAbright', '400Mpc_UVbright'/)
    ...
    INTEGER, PARAMETER :: N_FOREST = 8 !8 for 100Mpc, 512 for 400Mpc box !Makiya
...
!   INTEGER, PARAMETER :: N_COLUMN = 53 ! # of total column - 8 (integer)
    INTEGER, PARAMETER :: N_COLUMN = 47 !53 ! # of total column - 8 (integer) !Makiya
    INTEGER, PARAMETER :: N_COLUMN_I = 5 ! # of integer column
...
!    fname_mind = trim(fbase_pos)//'400Mpc_2048'//'.00000000.'//trim(czout)  !Makiya
    fname_mind = trim(fbase_pos)//'100Mpc_512'//'.00000000.'//trim(czout)  !Makiya
...
!      fname_in = trim(fbase)//trim(cz)//'LAE'//trim(fend(ifile))
       fname_in = trim(fbase)//'SMGs2_'//trim(czout)//trim(fend(ifile))  !Makiya
...
!                    (xin(ic), ic=ic0+1,N_COLUMN)
                     (xin(ic), ic=1,13), (ixin(ic), ic=3,N_COLUMN_I), & !Makiya
                     (xin(ic), ic=13+1,N_COLUMN)
!               mpi = ixin(4) ! mpi(gal)
