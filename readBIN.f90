...
!   INTEGER, PARAMETER :: N_COLUMN = 53 ! # of total column - 8 (integer)
    INTEGER, PARAMETER :: N_COLUMN = 53 !53 ! # of total column - 8 (integer) !Makiya
...
!                   DO_iforest: DO WHILE(flag == 0)
                      iforest = 1
                      DO WHILE(iforest <= N_FOREST)
                      DO ihalo = 1, p(iforest)%nhalo
                          IF(p(iforest)%mpi(ihalo) == 4940618548)THEN
                             print *, p(iforest)%mpi(ihalo), iforest
                          ENDIF
